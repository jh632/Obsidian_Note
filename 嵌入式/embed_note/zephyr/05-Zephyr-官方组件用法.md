...
> 2026-09-01 由原《zbus》笔记扩展而来：把「zbus 单个组件」扩成 **「Zephyr 官方子系统用法总集」**。
> 已整理的组件给详细用法；还没整理的列出「一句话作用」占位，学到就补进来——这样以后 Settings / Sensor API / Work Queue 等就不会再散落到别的文件。
...
| 官方子系统 | 一句话作用 | 状态 |
| --- | --- | --- |
| 内核原语（线程 / 调度 / 定时器 / 中断 / 信号量 / 消息队列） | 多线程、同步、数据传递 | ✅ 见 [[01-Zephyr-核心API]] |
| Logging 日志 | 分级 / 模块化 / 运行时过滤日志 | ✅ 见 [[01-Zephyr-核心API]] |
| Shell | 串口交互式命令 | ✅ 见 [[01-Zephyr-核心API]] |
| **zbus 消息总线** | **发布订阅、模块解耦** | ✅ 见本文件 §2 |
...
| Mailbox | 跨线程消息收发 | ⏳ 待补充 |
| Pipe | 流式（大块）数据传递 | ⏳ 待补充 |
| Watchdog | 看门狗（系统复位保护） | ⏳ 待补充 |
| Flash / DFU（mcuboot） | 固件升级 | ⏳ 待补充 |
| 外设驱动 API（GPIO / I2C / SPI / UART / PWM） | 外设操作 | ⏳ 待补充 |

> 用法：点开 `✅` 的链接看细节；`⏳` 的占位项以后学习时，直接在本文件追加对应章节即可。
...
> 相关笔记：[[嵌入式设计模式-事件总线]]（含 VDED / iterable section 实现剖析） [[01-Zephyr-核心API]]
...
### 2.2 核心概念：Message / Channel / Observer
...
#### 2.3.2 定义消息（建议单独放 `app/include/app_messages.h`）
...
#### 2.3.3 定义 Observer（如 UI 用 Subscriber）
...
> Subscriber 收到的是 **Channel 通知（channel 指针）**，不是 Message 本身——所以读数据要分两步（见 2.3.6）。
...
### 2.8 zbus vs FreeRTOS Queue / k_msgq
...
### 2.9 设计建议：区分 Event 和 State（最容易混淆的点）
...
2. **Async Listener 其实也拷贝消息**。老师表格写 Async Listener"消息是否复制=否"，但源码中 async listener 结构带 `message_fifo`，官方文档也说明 net_buf pool 服务于 message subscriber **和 async listener**——因为回调在工作队列上下文执行，必须拷贝一份消息带过去。正确表格见第 2.2 节。
