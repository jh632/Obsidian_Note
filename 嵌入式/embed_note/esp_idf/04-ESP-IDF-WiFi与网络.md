...
> 2026-09-01 由原《wifi》《esp32射频竞争》《websocket》整理合并。

---

## 目录

1. [WiFi 三层架构](#1-wifi-三层架构)
2. [STA 初始化流程](#2-sta-初始化流程)
3. [WiFi API 速查表](#3-wifi-api-速查表)
4. [WiFi 事件](#4-wifi-事件)
5. [常见错误码](#5-常见错误码)
6. [单射频竞争问题排查](#6-单射频竞争问题排查)
...
ESP-IDF WiFi 驱动基于 `esp_netif` + `esp_event` + `esp_wifi` 三层架构，提供 Station (STA)、SoftAP、STA+AP 三种工作模式。所有 WiFi 操作均为**事件驱动**——不在回调中等待结果，而是注册事件处理函数响应状态变化。

```
esp_wifi（驱动层）→ esp_event（事件分发）→ esp_netif（TCP/IP 接口层）
```
...
**`wifi_init_config_t`** — 初始化参数，**必须**用 `WIFI_INIT_CONFIG_DEFAULT()` 赋值后再覆盖特定字段：
...
> 场景：APSTA 共存模式下扫描时，手机会"掉 AP"。原《esp32射频竞争》排查记录。

### 6.1 根因：单射频时分复用

ESP32 只有一个 **2.4GHz 射频前端**，WiFi（AP + STA）和蓝牙（BT + BLE）共用，同一时刻只能干一件事，ESP-IDF 用**时分复用（TDM）**调度。

### 6.2 为什么手机会切网

- AP 模式每 **100ms 发一次 Beacon 帧**宣告存活
- 手机连续 **5-10 个 Beacon 收不到**（0.5-1s）就认为 AP 挂了，启动"智能切换网络"
- 配网时 STA 扫描逐个信道发 Probe Request，**射频物理上切走了**，AP 所在信道上发不出 Beacon
- 蓝牙共存再分走一半时间片，问题更严重（实测扫描总耗时从 ~1.5s 变 ~5s）

### 6.3 修复：`home_chan_dwell_time`

`wifi_scan_config_t` 提供"回主页信道停留时间"，让 AP 喘口气发 Beacon：

```c
typedef struct {
    ...
    uint8_t home_chan_dwell_time;   // 默认 30ms，范围 30-150ms
    wifi_scan_channel_bitmap_t channel_bitmap;  // 可指定只扫哪些信道
} wifi_scan_config_t;
```

- **默认 30ms 太短**——只够发 1/3 个 Beacon，手机大概率收不到
- **设成 150ms**：每扫完一个非 AP 信道，回来停 150ms 发完整 Beacon，手机不断收到 Beacon 不会触发智能切换
...
ESP-IDF 的 WebSocket 客户端基于 `esp_websocket_client` 组件。协议层原理见 [[通信协议/websocket]]（⚠️ 该协议笔记尚未创建，原《websocket》笔记即为此占位链接）。
...
// 事件回调中处理 ESP_WEBSOCKET_EVENT_DATA 等
```

> 服务端/协议细节见 [[通信协议/websocket]]。
...
- [[02-ESP-IDF-核心外设速查]] — NVS 初始化、GPIO
- [[算法/指数退避]] — 断线重连退避策略
- [[通信协议/websocket]] — WebSocket 协议
